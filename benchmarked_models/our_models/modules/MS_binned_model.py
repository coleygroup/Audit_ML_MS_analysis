...
class MSBinnedModel(pl.LightningModule):
    ...
    def forward(self, binned_ms, adduct, CE, instrument, return_emb = False):
        ...
        if return_emb: 
            return FP_pred, binned_ms_pred, emb

        return FP_pred, binned_ms_pred 

    def cosine_similarity_loss(self, FP_pred, FP):

        # Normalize vectors
        FP_pred = F.normalize(FP_pred, dim=1)
        FP = F.normalize(FP, dim=1)

        cos_sim = (FP_pred * FP).sum(dim=1)
