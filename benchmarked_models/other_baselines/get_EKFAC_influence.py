...
def get_datasets(folder, params, top_k):
    ...
    test_results = sorted(test_results.items(), key = lambda item: item[1]["jaccard"], reverse = True)

    # Look at 3 different types of test - the good test, the bad test and random samples in the middle 
    test_ids = [str(r[0]).replace("tensor(", "").replace(")", "") for r in test_results]
    test_ids_to_analyze_good = test_ids[:top_k]
    test_ids_to_analyze_bad = test_ids[-top_k:]
    test_ids_to_analyze_random = random.sample(test_ids[top_k: len(test_ids) - top_k], min(top_k, len(test_results) - 2 * top_k))
    test_ids_to_analyze = test_ids_to_analyze_good + test_ids_to_analyze_bad + test_ids_to_analyze_random
    ...
    # clean up the ids
    train_files_to_analyze = [r.split("/")[-1].replace(".pkl", "").replace(".ms", "") for r in train_files_to_analyze]
    test_files_to_analyze = [r.split("/")[-1].replace(".pkl", "").replace(".ms", "") for r in test_files_to_analyze]
...
def get_influence_scores(folder, output_path, self_output_path, top_k):
    ...
    # Set up the parameters for the scores argument 
    score_kwargs = ScoreArguments(damping_factor = 1e-5)
...
if __name__ == "__main__":
    ...
    for dataset in os.listdir(folder):
        ...
        for checkpoint in os.listdir(dataset_folder):
            if "MS" not in checkpoint: continue
